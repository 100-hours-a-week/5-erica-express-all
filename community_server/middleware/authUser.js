...
import { getCommentModel } from "../model/comments.js";
...
export const modifyAuthUser = (req, res, next) => {
  if (!req.session.user.userId || !req.session.user) {
    return res
      .status(403)
      .json({ status: 403, message: "unauthorizaed", data: null });
  }
...
//게시물 작성자가 본인이 맞는지 확인
export const getPostUser = (req, res, next) => {
  if (!req.params.id) {
    return res
      .status(400)
  ...
  if (
    !req.session ||
    !req.session.user ||
    req.session.user.userId !== post.userId
  ) {
    return res
      .status(403)
      .json({ status: 403, message: "unauthorizaed", data: null });
...
//댓글 작성자가 본인이 맞는지 확인
export const getCommentUser = (req, res, next) => {
  ...
  if (
    !req.session ||
    !req.session.user ||
    req.session.user.userId !== comment.userId
  ) {
    return res
      .status(403)
      .json({ status: 403, message: "unauthorizaed", data: null });
